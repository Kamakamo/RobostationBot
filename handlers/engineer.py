...
async def save_comment_and_complete(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if sheets.update_request_status(request_id, "Завершена", comment=comment):
        await update.message.reply_text(f"✅ Заявка #{request_id} успешно завершена с вашим комментарием.")
...
async def cancel_completion(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    request_id = context.user_data.get('completing_request_id')
    await update.message.reply_text(f"Завершение заявки #{request_id} отменено. Она остается в статусе 'В работе'.")
    context.user_data.clear()
    return ConversationHandler.END
...
async def claim_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not sheets.update_request_status(request_id, "В работе", engineer_username_raw, engineer_name_raw):
        await query.answer("Произошла ошибка при обновлении статуса в таблице.", show_alert=True)
        return

    await query.answer("Вы взяли заявку в работу! Карточка задачи отправлена вам в личные сообщения.")
    ...
    await query.edit_message_text(text=new_text_in_group, reply_markup=None, parse_mode='MarkdownV2')

    text_for_pm = (
        f"Вы взяли в работу заявку \\#{helpers.escape_markdown(request_id)}\\.\n\n"
        f"{body}"
    )
    keyboard = [[InlineKeyboardButton(text=f"🏁 Завершить #{request_id}", callback_data=f"{c.CB_COMPLETE_PREFIX}{request_id}")]]
...
async def show_requests(update: Update, context: ContextTypes.DEFAULT_TYPE, status: str):
    ...
    await update.message.reply_text(f"*{helpers.escape_markdown(f'Заявки в статусе «{status}»')}:*", parse_mode='MarkdownV2')
